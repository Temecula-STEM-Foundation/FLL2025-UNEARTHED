...
def Rescale(val: int, in_min: int, in_max: int, out_min: int, out_max: int) -> int:
    ...
    if val > in_max:
        val = in_max
    retVal: float = out_min + (val - in_min) * (out_max - out_min) / (in_max - in_min)
    if retVal > out_max:
        retVal = out_max
    if retVal < out_min:
        retVal = out_min
...
def get_battery_percentage(voltage_mv: int) -> int:
    """
    Calculates the battery percentage based on the typical voltage range of the
    LEGO Prime Hub's 2-cell Li-Ion battery.
    - 8400 mV is considered fully charged (100%).
    - 6200 mV is considered empty (0%), near the hub's auto-shutdown level.
    Args:
        voltage_mv: The battery voltage in millivolts.
    Returns:
        The estimated battery charge as an integer percentage (0-100).
    """
    if voltage_mv is None:
        raise ValueError(
            "voltage_mv cannot be None. Failed to read battery voltage from the hub."
        )

    # Define the realistic voltage range
    full_voltage = 8400  # mV
    empty_voltage = 6200  # mV

    # Clamp the voltage to be within the expected range
    clamped_voltage = max(empty_voltage, min(voltage_mv, full_voltage))
